...
\citep{subramanian2005}; both are well validated for detecting a \emph{quantitative, overall}
...
become first-class objects. Doing this faithfully turns on two properties of the data ---
pathway coupling is \emph{multiplicative} (R2), and measurement error is
\emph{multiplicative}/signal-dependent (R1) --- which together rule out the routine
``log-transform then PCA'' recipe and instead call for a factorisation in linear space under a
scale-invariant loss. We give that argument in full in the Supplementary Material
(\code{supplement.tex}) and summarise the resulting method, \code{colorpath}, in
Section~\ref{sec:methods}: a linear-space IS/KL non-negative factorisation, with optional
saturation censoring and an optional ICA-style independence penalty.
