...
become first-class objects. What such a decomposition should optimise is fixed by two
...
their activity is. Scanning the 304 KEGG pathways on a striatal section (\code{scan\_pathways.py}),
the highest-scoring of the 295 with sufficient genes are the \emph{synaptic} pathways ---
\textbf{GABAergic, dopaminergic and glutamatergic synapse} --- followed by cAMP, calcium and
retrograde-endocannabinoid signalling and neuroactive ligand--receptor interaction: the screen
recovers the striatal neurotransmission biology with no annotation and reproduces the
supervised ranking above, two independent metrics in agreement. (KEGG gene-set overlap means
several lower hits, e.g.\ the addiction pathways, largely re-detect the same synaptic
programmes rather than independent biology.)
