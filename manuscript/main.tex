...
\code{colorpath}, a framework for \emph{spatial pathway-activity analysis} built around
...
behave?} Two properties of these data constrain the answer.
...
Sequencing-count modalities share this signal-dependent character --- sampling (Poisson, and
typically over-dispersed) noise whose variance grows with the mean. The conventional remedy
is to take logarithms, after which the error is approximately
...
coordinated, \emph{proportional} change: if two features A and B (metabolites or
transcripts, say) are co-regulated, doubling the local activity of the pathway scales both. In linear space a rank-1 component
